...
export default function HeroSection() {
  ...
  return (
    <section
      ref={sectionRef}
      id="about"
      className="relative flex items-start overflow-x-hidden bg-white px-5 pt-10 pb-14 sm:px-8 sm:pt-12 sm:pb-16 md:min-h-[calc(100vh-65px)] md:items-center md:pb-24 xl:px-10"
    >
      <div className="mx-auto grid w-full max-w-7xl gap-8 md:grid-cols-[minmax(0,1fr)_minmax(19rem,25rem)] md:items-center md:gap-x-12 md:gap-y-8 lg:grid-cols-[minmax(0,1.05fr)_minmax(22rem,26rem)] lg:gap-x-16 lg:gap-y-10">
        ...
        <div className="relative order-1 flex flex-col items-center text-center md:order-none md:items-start md:pt-2 md:text-left lg:pt-4">
          <span className="hero-label mb-4 inline-flex items-center gap-3 font-mono text-[clamp(0.64rem,1.8vw,0.72rem)] uppercase tracking-[0.18em] text-stone-400 sm:tracking-[0.26em]">
            <span className="h-px w-12 bg-stone-300 sm:w-16" />
            {messages.hero.issue}
          </span>

          <h1 className="hero-name font-editorial text-[clamp(2.8rem,9vw,5.8rem)] font-bold italic leading-[0.98] tracking-[-0.04em] text-zinc-950">
            Jakkapet
            <br />
            <span className="text-zinc-300">Ladnok</span>
          </h1>

          <div className="hero-divider mt-6 h-px w-full max-w-xs origin-left bg-zinc-200 sm:mt-8 md:max-w-sm" />

          <p className="hero-role mt-5 font-mono text-[clamp(0.78rem,1.7vw,0.9rem)] font-semibold uppercase tracking-[0.2em] text-zinc-950 sm:tracking-[0.24em]">
            {messages.hero.role}
          </p>

          <p className="hero-desc mt-5 max-w-[31rem] text-pretty text-[clamp(1.02rem,2vw,1.14rem)] leading-[1.9] text-zinc-600">
          ...
          <div className="mt-9 flex w-full flex-col items-stretch gap-3 sm:max-w-max sm:flex-row sm:items-center sm:justify-center sm:gap-4 md:justify-start">
            <a
              className="hero-btn flex h-11 w-full items-center justify-center gap-2 border border-zinc-950 bg-zinc-950 px-5 text-[clamp(0.72rem,1.9vw,0.82rem)] font-semibold uppercase tracking-[0.1em] text-white transition-all duration-300 hover:bg-white hover:text-zinc-950 sm:h-12 sm:w-auto sm:px-7 sm:tracking-[0.12em]"
        ...
        <div className="hero-image order-3 relative mx-auto w-full max-w-[18rem] md:order-none md:col-start-2 md:row-span-2 md:row-start-1 md:max-w-[18rem] md:justify-self-end lg:max-w-[20rem] xl:max-w-[22rem]">
          <div className="pointer-events-none absolute left-1/2 top-8 h-52 w-52 -translate-x-1/2 rounded-full bg-stone-100 blur-3xl md:left-auto md:right-0 md:top-14 md:translate-x-0" />
          <div className="pointer-events-none absolute -left-2 top-10 h-[86%] w-[86%] -rotate-[2.5deg] border border-stone-100 bg-stone-50/85 md:-left-4" />
          <div className="pointer-events-none absolute right-0 top-0 h-[90%] w-[80%] rotate-[1.5deg] border border-zinc-200/70 bg-white/92 md:-right-2" />

          <div className="relative z-10 px-3 pt-3 sm:px-4 sm:pt-4">
            <div className="relative overflow-hidden bg-white shadow-[0_26px_55px_rgba(24,24,27,0.1)]">
          ...
          <div className="relative z-20 mt-5 flex items-end justify-between gap-4">
            <div className="border-l border-stone-200 pl-3">
              <span className="block font-editorial text-[clamp(1.15rem,2.4vw,1.45rem)] italic text-stone-300">Portrait</span>
              <span className="mt-1 block font-mono text-[0.58rem] uppercase tracking-[0.22em] text-stone-400 sm:text-[0.62rem]">
                Soft Editorial
              </span>
            </div>
            <p className="pr-2 text-right font-mono text-[clamp(0.56rem,1.6vw,0.65rem)] uppercase tracking-[0.16em] text-stone-400 sm:tracking-[0.2em]">
